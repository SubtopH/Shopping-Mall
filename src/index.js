...
import shangou from './shangou.art'
import jiadian from './jiadian.art'
import jiadian2 from './jiadian2.art'
import jiadian3 from './jiadian3.art'
import jiadian4 from './jiadian4.art'
import jia from './jia.art'
...
$(function(){
	$.get('./src/data.json',function(res){
		...
		// 数据加入到模板
		var ban = banner(data)
		// 渲染好的页面加入index.html的 .ban里面
		$('.ban').append(ban)

		var shan = shangou(data)
		...
		// 在渲染其他数据，但是要用prepend追加在4个盒子前面
		var jiad = jiadian(data)
		$('.fenye').eq(0).prepend(jiad)
		var jiad2 = jiadian2(data)
		$('.fenye').eq(1).prepend(jiad2)
		var jiad3 = jiadian3(data)
		$('.fenye').eq(2).prepend(jiad3)
		var jiad4 = jiadian4(data)
		$('.fenye').eq(3).prepend(jiad4)
	})
...
function lunbo(){
			...
			//			判断s范围
				s=s==5?0:s;
			//			banner显示隐藏
				$('.guanghz .guangzhong .ban a').eq(s).fadeIn(1000).siblings('a').fadeOut(1000);
			//			按键样式变化
			...
			//移入时清除定时器
			$('.guanghz .guangzhong .ban ').mouseover(function(){
				clearInterval(times);
			});
			...
			//移出图片时恢复定时器
			$('.guanghz .guangzhong .ban ').mouseleave(function(){
			...
			//			获得点击序号
				s = $(this).index();
			//			 banner和按键切换样式
				$('.guanghz .guangzhong .ban a').eq(s).fadeIn(200).siblings('a').fadeOut(200);
				$('.guanghz .guangzhong ul li').eq(s).css('background','rgba(255,255,255,0.5)').siblings('li').css('background','rgba(0,0,0,0.3)');
			})
			//点击左右按键切换
			...
			//			有按键时,序号++
				...
				s=s==5?0:s;
			//			样式变化
				$('.guanghz .guangzhong .ban a').eq(s).stop().fadeIn(200).siblings('a').fadeOut(200);
				$('.guanghz .guangzhong ul li').eq(s).css('background','rgba(255,255,255,0.5)').siblings('li').css('background','rgba(0,0,0,0.3)');
...
//全局变量
	...
	$('.zyzuo i').click(function(){
			...
			//	计算剩余分
			var newshi = zongshi%(60*60*1000);
			//	计算分
			var fen = parseInt(newshi/(60*1000));
			//	计算剩余秒
			var newshi = zongshi%(60*1000);
			//	计算秒
...
//	根据对应序号，计算对应元素里底部按键个数
		var z=$('.nei .neinei .neineihz').eq(hz).find('ul li').length
	//	点击左侧序号加加
		n[hz]++;
	//	根据底部按键个数，判断序号范围
